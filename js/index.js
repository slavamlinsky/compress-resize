...
let resizingFactor = 0.75;
let quality = 0.75;

//document.querySelector("#originalsize").innerHTML = bytesToSize(originalImage.width);
...
fileInput.addEventListener("change", async (e) => {
  ...
  originalImage.addEventListener("load", () => {
    ...
    //lert(document.querySelector("#upload"))
    console.log(document.querySelector("#upload").files[0]);    
    document.querySelector("#originalsize").innerHTML = bytesToSize(document.querySelector("#upload").files[0].size) + " (" + originalImage.width + "*" + originalImage.height + ")";
  });
...
resizingElement.oninput = (e) => {
  ...
  document.querySelector('#percent').innerHTML=Math.round(resizingFactor*100);
  compressImage(originalImage, resizingFactor, quality);
...
qualityElement.oninput = (e) => {
  ...
  document.querySelector('#quality').innerHTML=Math.round(quality*100);
  compressImage(originalImage, resizingFactor, quality);
  ...
  canvas.toBlob(
    (blob) => {
      if (blob) {
        ...
        document.querySelector("#size").innerHTML = bytesToSize(blob.size) + " (" + Math.round(canvasWidth) + "*" + Math.round(canvasHeight) + ")";;
      }
    },
    "image/jpeg",
    quality
